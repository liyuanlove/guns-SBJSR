...
public class ShiroKit {
    ...
    public static String principal() {
        if (getSubject() != null) {
            //等价于getSubject().getPrincipals().getPrimaryPrincipal()
    ...
    public static List<Integer> getDeptDataScope() {
        ...
        List<Integer> list = ConstantFactory.me().getSubDeptId(deptId);
        list.add(deptId);
        return list;
    }
     ...
     * 权限断言
     */
    public static void assertAuth(Integer userId, User user) {
        //超级管理员在操作
        if (isAdmin()) {
            return;
        }
